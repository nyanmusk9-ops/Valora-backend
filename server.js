...
app.get("/api/matches/:matchId/presence",(req,res)=>{try{const db=loadDb();const m=db.matches[req.params.matchId];if(!m)throw new Error("Match not found");const now=Date.now();const players=[m.playerA,m.playerB].map(id=>{const u=db.users[id];const ls=(u&&u.lastSeen)||0;return{id,lastSeen:ls,agoMs:now-ls,online:(now-ls)<ONLINE_WINDOW_MS};});res.json({success:true,now,status:m.status,players});}catch(e){res.status(400).json({success:false,error:e.message});}});
